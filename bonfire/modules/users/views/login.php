  ...
  <div id="login" style="margin-top:20px">
   ...
   <?php echo form_open(LOGIN_URL, array('autocomplete' => 'off', 'class' => 'form-horizontal')); ?>
    ...
    <div class="form-group <?php echo iif( form_error('login') , 'error') ;?>">
      <input style="width: 95%" type="text" class="form-control" name="login" id="login_value" value="<?php echo set_value('login'); ?>" tabindex="1" placeholder="<?php echo $this->settings_lib->item('auth.login_type') == 'both' ? lang('bf_username') .'/'. lang('bf_email') : ucwords($this->settings_lib->item('auth.login_type')) ?>" />
    </div>
    ...
    <?php if ($this->settings_lib->item('auth.allow_remember')) : ?>
     <div class="form-group">
       <label class="checkbox controls" for="remember_me">
    ...
    <div class="form-group">
      <input class="btn btn-large btn-primary form-control" type="submit" name="log-me-in" id="submit" value="<?php e(lang('us_let_me_in')); ?>" tabindex="5" />
    </div>
   <?php echo form_close(); ?>
